...
const CONFIG_PATH = path.join(__dirname, "config", "product-areas.json");
...
/**
 * Merge seed config + roster into a single in-memory productAreas object.
...
async function saveRoster(logger = null) {
  ...
  for (const area of productAreas.areas) {
    roster[area.id] = {
      leads: area.leads || [],
...
/**
 * Save the seed config file (src/config/product-areas.json).
...
async function saveConfig(logger = null) {
  ...
  seedConfig.areas = productAreas.areas.map((area) => ({
    id: area.id,
    name: area.name,
  ...
  if (logger) {
    logger.info(`[Config] Saved ${seedConfig.areas.length} product area(s) to ${CONFIG_PATH}`);
  }
}
 ...
 */
export async function saveProductAreas(logger = null) {
...
/**
 * Load product areas from disk (seed config + roster), merge into memory.
 */
export async function loadProductAreas(logger = null) {
  // 1. Load seed config
  try {
    ...
    if (logger) {
      logger.info(`[Config] Loaded ${seedConfig.areas.length} product area(s) from seed config`);
    }
  } catch (err) {
    if (err.code === "ENOENT") {
      seedConfig = { areas: [] };
  ...
  if (logger) {
    logger.info(`[Config] Merged ${productAreas.areas.length} product area(s) into memory`);
  }
...
export function getAllProductAreas() {
  return [...productAreas.areas];
}

export function getProductAreaById(id) {
  return productAreas.areas.find((a) => a.id === id) || null;
}

export function getProductAreaByName(name) {
  const lower = name.toLowerCase();
  return productAreas.areas.find((a) => a.name.toLowerCase() === lower) || null;
}

// ─── Lead / Team Member helpers ─────────────────────────────────────────────

export function getLeadUserIds(areaId) {
  const area = getProductAreaById(areaId);
  if (!area) return [];
...
export async function addLead(areaId, userId, description = "", logger = null) {
  const area = getProductAreaById(areaId);
  if (!area) throw new Error(`Product area with ID "${areaId}" not found`);
...
// ─── Product area CRUD (write to seed config + roster) ──────────────────────

export async function addProductArea({ name, description = "", notionPageId, ownerUserIds = [], keywords = [] }, logger = null) {
  if (!name || !notionPageId) {
    throw new Error("Name and notionPageId are required");
  }

  if (getProductAreaByName(name)) {
    throw new Error(`Product area "${name}" already exists`);
  }
  ...
  productAreas.areas.push(area);
  await saveConfig(logger);
  await saveRoster(logger);

  if (logger) {
    logger.info(`[Config] Added product area: "${area.name}" (ID: ${area.id})`);
...
export async function removeProductArea(id, logger = null) {
  const index = productAreas.areas.findIndex((a) => a.id === id);
  if (index === -1) {
    if (logger) logger.warn(`[Config] Attempted to remove product area with ID "${id}" but not found`);
    return false;
  }

  const area = productAreas.areas[index];
  productAreas.areas.splice(index, 1);
  delete roster[id];

  await saveConfig(logger);
  await saveRoster(logger);

  if (logger) logger.info(`[Config] Removed product area: "${area.name}" (ID: ${area.id})`);
  return true;
}

export async function updateProductArea(id, updates, logger = null) {
  const area = getProductAreaById(id);
  if (!area) {
    throw new Error(`Product area with ID "${id}" not found`);
  }

  if (updates.name && updates.name.toLowerCase() !== area.name.toLowerCase()) {
    if (getProductAreaByName(updates.name)) {
      throw new Error(`Product area "${updates.name}" already exists`);
    }
  }
  ...
  if (logger) {
    logger.info(`[Config] Updated product area: "${area.name}" (ID: ${area.id})`);
